...
$RoboFolders = Import-CSV -Path C:\scripts\folders.csv

$RoboFolders | ForEach-Parallel -Throttle 8 {
      $Log = $_.Name
      $RoboSwitches = "/MIR /z /R:0 /w:0 /xo /fft /np /MT:8 /Log:C:\scripts\Logs\$($Log).log"
      $Source = ( '"' + $_.Source + '"')
      $Target = ( '"' + $_.Target + '"')

      Start-Process -ArgumentList "$Source $Target $RoboSwitches" robocopy.exe -WAIT
      }